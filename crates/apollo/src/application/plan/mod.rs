...
thread_local! {
    ...
    pub(super) static AXIS_BUF: RefCell<Vec<Complex64>> = const { RefCell::new(Vec::new()) };
    /// Thread-local scratch workspace used by `rustfft::Fft::process_with_scratch`.
    pub(super) static AXIS_SCRATCH: RefCell<Vec<Complex64>> = const { RefCell::new(Vec::new()) };
    /// Thread-local scratch buffer for `Complex32` axis transforms.
    pub(super) static AXIS_BUF_32: RefCell<Vec<Complex32>> = const { RefCell::new(Vec::new()) };
    /// Thread-local scratch workspace for `Complex32` FFT execution.
    pub(super) static AXIS_SCRATCH_32: RefCell<Vec<Complex32>> = const { RefCell::new(Vec::new()) };
    /// Thread-local transposed scratch buffer for large strided sweeps.
    pub(super) static AXIS_BUF_2D: RefCell<Vec<Complex64>> = const { RefCell::new(Vec::new()) };
    /// Thread-local transposed scratch buffer for `Complex32` strided sweeps.
    pub(super) static AXIS_BUF_2D_32: RefCell<Vec<Complex32>> = const { RefCell::new(Vec::new()) };
    /// Thread-local real-valued scratch buffer used by the RFFT Z pass.
    pub(super) static RFFT_REAL_BUF: RefCell<Vec<f64>> = const { RefCell::new(Vec::new()) };
    /// Thread-local half-spectrum scratch used by Hermitian-aware inverse paths.
    pub(super) static HALF_SPECTRUM_BUF: RefCell<Vec<Complex64>> = const { RefCell::new(Vec::new()) };
    /// Thread-local full-volume workspace used to avoid repeated complex-volume allocations.
    pub(super) static VOLUME_COMPLEX_BUF: RefCell<Vec<Complex64>> = const { RefCell::new(Vec::new()) };
}
