...
fn radix64_inplace_64(data: &mut [Complex64], inverse: bool, twiddles: Option<&[Complex64]>) {
    const RADIX: usize = 64;
...
fn radix64_inplace_32(data: &mut [Complex32], inverse: bool, twiddles: Option<&[Complex32]>) {
    const RADIX: usize = 64;
    debug_assert!(data.len().is_power_of_two());
    debug_assert!((data.len().trailing_zeros() as usize) % 6 == 0);
    if data.len() <= 1 {
