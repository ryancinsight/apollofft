...
fn is_power_of_thirty_two(n: usize) -> bool {
    n.is_power_of_two() && (n.trailing_zeros() % 5 == 0)
}

#[inline]
/// Forward FFT (unnormalized) for power-of-thirty-two lengths using caller-provided twiddles.
pub fn forward_inplace_64_with_twiddles(data: &mut [Complex64], twiddles: &[Complex64]) {
    ...
    radix32_inplace_64(data, false, Some(twiddles));
}

#[inline]
/// Inverse FFT (unnormalized) for power-of-thirty-two lengths using caller-provided twiddles.
pub fn inverse_inplace_unnorm_64_with_twiddles(data: &mut [Complex64], twiddles: &[Complex64]) {
    if data.len() <= 1 {
        return;
    }
    debug_assert!(is_power_of_thirty_two(data.len()));
    radix32_inplace_64(data, true, Some(twiddles));
}

#[inline]
/// Inverse FFT normalized by 1/N for power-of-thirty-two lengths using caller-provided twiddles.
...
/// Forward FFT (unnormalized) for power-of-thirty-two lengths.
...
#[inline]
/// Forward FFT (unnormalized, f32) for power-of-thirty-two lengths using caller-provided twiddles.
pub fn forward_inplace_32_with_twiddles(data: &mut [Complex32], twiddles: &[Complex32]) {
    if data.len() <= 1 {
        return;
    }
    debug_assert!(is_power_of_thirty_two(data.len()));
    radix32_inplace_32(data, false, Some(twiddles));
}

#[inline]
/// Inverse FFT (unnormalized, f32) for power-of-thirty-two lengths using caller-provided twiddles.
pub fn inverse_inplace_unnorm_32_with_twiddles(data: &mut [Complex32], twiddles: &[Complex32]) {
    if data.len() <= 1 {
        return;
    }
    debug_assert!(is_power_of_thirty_two(data.len()));
    radix32_inplace_32(data, true, Some(twiddles));
}

#[inline]
/// Inverse FFT normalized by 1/N (f32) for power-of-thirty-two lengths using caller-provided twiddles.
...
/// Forward FFT (unnormalized, f32) for power-of-thirty-two lengths.
...
/// Forward FFT (f16 storage) for power-of-thirty-two lengths via f32 upcast.
...
/// Forward FFT (f16 storage) for power-of-thirty-two lengths using caller-provided twiddles.
...
fn radix32_inplace_64(data: &mut [Complex64], inverse: bool, twiddles: Option<&[Complex64]>) {
    const RADIX: usize = 32;
...
fn radix32_inplace_32(data: &mut [Complex32], inverse: bool, twiddles: Option<&[Complex32]>) {
    const RADIX: usize = 32;
    debug_assert!(data.len().is_power_of_two());
    debug_assert!((data.len().trailing_zeros() as usize) % 5 == 0);
    if data.len() <= 1 {
