...
fn sft_direct_dft(@builtin(global_invocation_id) gid: vec3<u32>) {
    let row = gid.x;
    if row >= params.len {
        return;
    }
    ...
    let scale = select(1.0, 1.0 / f32(params.len), params.mode == 1u);
    var sum = Complex32(0.0, 0.0);

    for (var col: u32 = 0u; col < params.len; col = col + 1u) {
        let angle = sign * TAU * f32((row * col) % params.len) / f32(params.len);
