...
pub enum WgpuError {
    ...
    /// Plan length is empty or not a power of two.
    #[error("invalid FWHT WGPU plan length {len}: {message}")]
    InvalidLength {
    ...
    #[error("precision profile does not match typed FWHT-WGPU storage")]
